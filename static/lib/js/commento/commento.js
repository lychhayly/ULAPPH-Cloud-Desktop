(function(global, document) {
    ...
    var _showdownUrl = "/lib/js/commento/showdown.min.js";
    var _spectreUrl = "/lib/css/commento/spectre.min.css";
    var _commentoCssUrl = "/lib/css/commento/commento.css";
    var _serverUrl = '';
    var _honeypot = false;
    var _api = {};
    ...
    var _makeCards = function(parentMap, cur) {
        ...
        currentParent.forEach(function(comment) {
            ...
            h5.innerHTML = comment.name + ' ' + '<a href=\"/social?SO_FUNC=ViewPeople&UID=' + comment.uid + '\" title=\"View Profile\" target=\"prof\"><img src=\"' + comment.profile + '\" width=30 height=30></a>' + '<a href=\"https://www.google.com/maps/place/' + comment.latlon + '\" title=\"Show Map\" target=\"loc\">' + '<img src=\"/img/mapmarker.png\" width=25 height=25></a>' + '<img src=\"' + comment.country + '\" width=30 height=20>';
			var cont = comment.comment;
			body.innerHTML = _showdownConverter.makeHtml(linkify(cont));
    ...
    var _postRoot = function() {
		...
		var localStorageKey =  location.host + "-commento-" + uid;
		console.log("localStorageKey: "+localStorageKey);
		console.log("rootNameValue: "+rootNameValue);
		localStorage.setItem(localStorageKey, rootNameValue);
		//--
        var data;
    ...
    var _showReply = function(id) {
		...
		var uid = document.getElementById("uid").value;
		var localStorageKey =  location.host + "-commento-" + uid;
		var storedNick = localStorage.getItem(localStorageKey);
		console.log("storedNick: "+storedNick);
		if (storedNick == "" || storedNick == undefined) {
			setAttr(name, "placeholder", "Name");
		} else {
			//ulapph
			console.log("storedNick set as value");
			setAttr(name, "value", storedNick);
		}
		setAttr(name, "placeholder", "Name");
    ...
    Commento.init = function(configuration) {
        ...
        _api.get = _serverUrl + '/comments?C_FUNC=get';
        _api.create = _serverUrl + '/comments?C_FUNC=create';
		//_api.create = _serverUrl + '/captcha?CC_FUNC=DISP&R=COMMENT';
