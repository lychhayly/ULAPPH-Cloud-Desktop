...
function onError(err) {
	...
	innerHTML = "<a href='/infodb?DB_FUNC=ULAPPH-NOTIFICATIONS-LOG&SID=ULAPPH-NOTIFICATIONS-LOG' target='notifications'><img src='/img/channel-error.png' width=60 height=60></img></a></a>CHANNEL ERROR. [<a href='#' onclick=resetChannel(" + aUser.value + "); return false;>Reset Channel</a>]";
	alertifyThis(innerHTML);
	//reConnect();
};
...
function onClose() {
	...
	innerHTML = "<a href='/infodb?DB_FUNC=ULAPPH-NOTIFICATIONS-LOG&SID=ULAPPH-NOTIFICATIONS-LOG' target='notifications'><img src='/img/channel-disconnected.png' width=60 height=60></img></a>CHANNEL DISCONNECTED. [<a href='#' onclick=resetChannel(" + aUser.value + "); return false;>Reset Channel</a>]";
	alertifyThis(innerHTML);
	//reConnect();
};
...
function resetChannel(uid) {
	...
	var message = "<img src='/img/pinned.png' width=30 height=30> Reset channel requested.";
	  ...
	  if (cxhr2.readyState==4 && cxhr2.status==200)
		...
		var currVal = cxhr2.responseText;
			if (currVal != "") {
				...
				var e = document.getElementById("channel-area");
				var currHTML = e.innerHTML;
				var message = "<img src='/img/success.png' width=30 height=30> Channel refresh successful!";
				e.innerHTML = message + "<hr>" + currHTML;
				return;
			}
		}
...
function putMarker() {
	var e = document.getElementById("channel-area");
	var currHTML = e.innerHTML;
	var message = "<br><img src=\"/img/divider-line.png\">";
	e.innerHTML = message + "<hr>" + currHTML;	
	return;
}
...
function dispSavedNotifs() {
	...
	if(typeof(Storage) !== "undefined") {
		currNotifsLS = localStorage['notificationsStorage'+a.value];
		var divider = "<br><a href='#' onclick='resetNotifStorage(); return false;' title='Click to reset storage'><img src=\"/img/divider-line.png\"></a>";
		var currHTML = e.innerHTML;
...
function soundOnOffStream() {
	...
	var res = e.src;
	var soundOn = false;
	var n = res.indexOf("/img/sound-on.png");
	if (n > 0) {
		soundOn = true;
	} else {
		soundOn = false;
