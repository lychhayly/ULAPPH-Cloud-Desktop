function winOnOff() {
	...
	var n = res.indexOf("/img/jswm-shide2.png");
	if (n > 0) {
		winStat = true;
	} else {
		winStat = false;
...
function uwmOnOff() {
	...
	var uwmStat = false;
	//alertify.error(thisSoundSrc);
	var n = res.indexOf("/img/uwm_plus.png");
	if (n > 0) {
		uwmStat = true;
	} else {
		uwmStat = false;
...
function uwmQuickSearch() {
	...
	if(typeof(Storage) !== "undefined") {
		...
		if (idxContent != "" && idxContent != undefined) {
			...
			var jswmstr = "'" + "LS" + "', 500, 300, 'left', 'top', {title: '" + tgt2 + "', icon: '/img/jswm-web.png'}";
	...
	if (idxContent == "" || idxContent == undefined) {
		...
		var jswmstr = "'" + tgt + "', 500, 300, 'left', 'top', {title: '" + tgt2 + "', icon: '/img/jswm-web.png'}";
