...
JSWindow.prototype.openTab = function(contents, force)
{
	...
	var jsTab = new JSTab(this, tabContents, this.tabs.length + 'long tab name ' + this.tabs.length, '/img/jswm-tab.png');
	this.addTab(jsTab)
};
...
function soundOpen() {
	...
	if (isEdge == true || isIE == true || isSafari == true) {
		console.log("isSafari/IE/Edge");
		soundManager.createSound({
			id: 'mySoundOJ',
			volume: 50,
			url: root + "/audio/fullscreen.mp3"
		});
...
function soundClose() {
	...
	if (isEdge == true || isIE == true || isSafari == true) {
		soundManager.createSound({
			id: 'mySoundC',
			volume: 50,
			url: root + "/audio/closeWindow.mp3"
		});
